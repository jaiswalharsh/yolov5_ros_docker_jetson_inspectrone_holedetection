...
from inspectrone_msgs.msg import BoundingBox, BoundingBoxes
import cv2
...
class detectManager:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber(
            self.image_topic, Image, self.imageCb, queue_size=1, buff_size=2**28)

        # Define publishers
        self.pub_ = rospy.Publisher(
            self.detected_objects_topic, BoundingBoxes, queue_size=1
            )
        self.pub_viz_ = rospy.Publisher(
            self.published_image_topic, Image, queue_size=1)
    ...
    def imageCb(self, data):
        ...
        timeStamp5 = time_sync()

        conversionTime = timeStamp2 - timeStamp1
        preProcessing = timeStamp4 - timeStamp3
        inferenceTime = timeStamp5 - timeStamp4
        # {t2 - t1:.3f}
        # print(f'{s}Done. ({t2 - t1:.3f}s)')

        print("conversion time:", conversionTime)
        print("preProcessing time:", preProcessing)
        print("inferenceTime time:", inferenceTime)
    ...
    def detect(self, opencv_img, data, save_img=False):
        ...
        #TODO: Scaling the img. Why it doesn't belong to preprocess method?????
        ...
        print("img_time_stamp:", data.header.stamp)
        print("BB_img_time_stamp:", detection_results.image_header.stamp)
